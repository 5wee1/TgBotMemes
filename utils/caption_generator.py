...
— Конкретность убивает банальность. "Когда сдал проект в 4 утра и вспомнил что не то задание" > "когда всё идёт не так"
— Неожиданный поворот. Начни в одну сторону, закончи в другую.
— Антикульминация. Огромная проблема → смешное мелкое решение (или наоборот).
— Внутренний монолог персонажа. Говори от лица того кто на картинке.
...
— ЗАПРЕЩЕНО: "когда понимаешь что...", "я когда...", любые шаблонные зачины. Ищи неожиданный угол.

Твой процесс:
1. АНАЛИЗ — какая здесь настоящая боль, абсурд или узнаваемая ситуация?
2. ВАРИАНТЫ — напиши 3 подписи используя РАЗНЫЕ техники из правил выше
3. ВЫБОР — какая из трёх реально смешная и почему (будь честен, отбрось слабые)
4. ФИНАЛ — финальная подпись (русский, 1-2 строки, макс 90 символов, без кавычек, без эмодзи)
5. КАРТИНКА — визуальная сцена для AI-генератора (английский, макс 70 слов, НОЛЬ текста/букв/надписей)

Отвечай ТОЛЬКО валидным JSON без markdown-обёртки:
{
  "analysis": "суть юмора в этой теме",
  "captions": ["вариант 1", "вариант 2", "вариант 3"],
  "best_caption_reason": "почему выбранный вариант лучший",
  ...
  "image_prompt": "visual scene description in English"
    ...
    try:
        ...
        logger.debug("Meme reasoning: %s", data.get("best_caption_reason", ""))
        if not image_prompt:
